...
export class Shader {
    ...
    constructor(ctx: WebGL2RenderingContext, name: string, vertex: string, fragment: string) {
        this.name = name;
        this.ctx = ctx;
    ...
    public uniform(name: string, value: Uniform) {
        ...
        if (found) {
            if (found.type !== value.type) {
                throw new Error(`uniform type mismatch: 'received: ${value.type} and have: ${found.type}'`);
            }
    ...
    private updateUniform(name: string, uniform: TaggedUniform, update: Uniform) {
        ...
        switch (uniform.type) {
            ...
            case 'vec2': {
                this.ctx.uniform2f(uniform.location, uniform.value[0], uniform.value[1]);
                break;
            }
            case 'vec3': {
                this.ctx.uniform3f(uniform.location, uniform.value[0], uniform.value[1], uniform.value[2]);
                break;
            }
            case 'vec4': {
                this.ctx.uniform4f(uniform.location, uniform.value[0], uniform.value[1], uniform.value[2], uniform.value[3]);
                break;
            }
            case 'mat2': {
