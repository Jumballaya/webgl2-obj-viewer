      ...
      switch (tokens[0].type) {
        ...
        case 'albedo-map': {
          if (tokens[1].type === 'string') {
            ...
            this.texturesToLoad.push({ name , path: `${this.baseDir}${tokens[1].value}` })
          }
          break;
        }
        case 'specular-map': {
        ...
        case 'ambient': {
          this.materials[this.currentMaterial].ambient = [tokens[1].value as number, tokens[2].value as number, tokens[3].value as number]
          break;
        }
        case 'diffuse': {
          this.materials[this.currentMaterial].diffuse = [tokens[1].value as number, tokens[2].value as number, tokens[3].value as number]
          break;
        }
        case 'emissive': {
          this.materials[this.currentMaterial].emissive = [tokens[1].value as number, tokens[2].value as number, tokens[3].value as number]
          break;
        }
        case 'specular': {
          this.materials[this.currentMaterial].specular = [tokens[1].value as number, tokens[2].value as number, tokens[3].value as number]
          break;
        }
        case 'dissolve': {
